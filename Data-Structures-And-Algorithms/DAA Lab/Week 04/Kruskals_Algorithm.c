...
/* Github Desciption: Kruskal's Algorithm finds the Minimum Cost Spanning Tree of a Graph (say G). A Spanning Tree is a tree in which all
nodes of G and edges ones less than that in G are present and most importantly *doesn't contain a cycle* . A Minimum Cost Spanning Tree is a
spanning tree in which the cost(sum of edge weights) is minimum.
Kruskal's Algorithm uses Greedy Approach to find the Minimum Cost Spanning Tree. It first sorts all the edges of  G in ascending order of
weights and then adds an edge to the solution vector/array if the 2 edges are not forming a cycle.
Kruskal's Algorithm can be done in an efficient way using the Union-Find Algorithm. Union-Find has 2 main functions Union and Find. Union
merges the components of 2 elements and Find returns the component in which the element is present. */
...
int main()
{
    int n, ed, i, j, minimumCost = 0; // n->number of nodes/vertices, ed->number of edges
    printf("No. Of Nodes and Edges: ");
    scanf("%d%d",&n, &ed);
    int comp[n];
    edges *e = (edges*)malloc(ed*sizeof(edges));
    solution *kruskal = (solution*)malloc((n-1)*sizeof(solution)); // Spanning Tree Contains only n-1 edges
    ...
    for(i = 0 ; i < n-1 ; i++)
        printf("[%d, %d] Weight: %d\n", kruskal[i].v1, kruskal[i].v2, kruskal[i].wt);
