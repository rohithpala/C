...
void SingleSourceShortestPath(int n, int adj_mat[][n], int sourceVertex){
    int i, j, k, shortestPath[n], alreadySelected[n], min_dist, min_dist_vertex;
    ...
    for(i = 0 ; i < n ; i++){
        ...
        alreadySelected[min_dist_vertex] = 1;

        // Relaxation
        for (j = 0; j < n; j++){
            // Select unalreadySelected vertices, adjacent to min_dist_vertex and
            // distance from source to this vertex is not infinity
            // Also apply the relaxation procedure
                if(!alreadySelected[j] && adj_mat[min_dist_vertex][j] && shortestPath[min_dist_vertex] != INT_MAX && 
